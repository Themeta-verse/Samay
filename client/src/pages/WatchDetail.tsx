...
export default function WatchDetail() {
  ...
  return <main ref={pageRef} className={`watch-detail watch-detail--${watch.slug}`}>
    ...
    <section className="product-detail__section section-paper" data-watch-reveal><div className="product-feature"><figure className="product-feature__image"><ImageWithFallback src={detail.movement.image} fallbackSrc={watch.image} fallbackLabel={`${watch.name} movement study unavailable`} alt={detail.movement.alt} decoding="async" loading="eager" /></figure><div className="product-feature__copy"><SectionLabel index="04">{detail.movement.label}</SectionLabel><h2>{detail.movement.title}<br /><em>{detail.movement.emphasis}</em></h2><p>{detail.movement.body}</p></div></div></section>

    <section className="product-detail__section section-paper" data-watch-reveal><div className="product-feature product-feature--reverse"><div className="product-feature__copy"><SectionLabel index="05">Material decision</SectionLabel><h2>{detail.material.title}<br /><em>{detail.material.emphasis}</em></h2><p>{detail.material.body}</p></div><figure className="product-feature__image"><ImageWithFallback src={detail.material.image} fallbackSrc={watch.image} fallbackLabel={`${watch.name} material study unavailable`} alt={detail.material.alt} decoding="async" loading="eager" /></figure></div></section>

    <section className="product-detail__section product-craft" data-watch-reveal><SectionLabel index="06">The considered details</SectionLabel><div className="product-craft__grid">{detail.craft.map((item) => <article className="product-craft__card" key={item.title}><ImageWithFallback src={item.image} fallbackSrc={watch.image} fallbackLabel={`${watch.name} detail study unavailable`} alt={item.alt} decoding="async" loading="eager" /><h3>{item.title}</h3><p>{item.body}</p></article>)}</div></section>
    ...
    <section className="related section-paper" data-watch-reveal><div className="section-wrap"><SectionLabel index="08">Complete the collection</SectionLabel><div className="related__head"><h2>{chapter.relatedTitle}<br /><em>{chapter.relatedEmphasis}</em></h2><ArrowLink href="/collection">See the collection</ArrowLink></div><div className="related__grid">{watches.filter((item) => item.slug !== watch.slug).map((item) => <Link key={item.slug} href={`/watch/${item.slug}`} className="related-card"><ImageWithFallback src={item.image} fallbackLabel={`${item.name} campaign image unavailable`} alt={`${item.name} watch`} decoding="async" loading="eager" /><span className="eyebrow">{item.reference}</span><h3>{item.name}</h3><ArrowUpRight size={17} strokeWidth={1.1} /></Link>)}</div></div></section>
  </main>;
}
