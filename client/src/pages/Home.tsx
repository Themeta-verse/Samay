...
export default function Home() {
  return (
    <div className="home-page">
      ...
      <section className="journal-preview section-paper"><div className="section-wrap"><SectionLabel index="04">The journal</SectionLabel><div className="journal-preview__head"><h2>Notes from<br /><em>the workbench.</em></h2><ArrowLink href="/journal">Read the journal</ArrowLink></div><div className="journal-preview__grid">{journalEntries.map((entry) => <Link key={entry.slug} href={`/journal/${entry.slug}`} className="journal-tile"><div className="journal-tile__image"><img src={entry.image} alt="" loading="lazy" /></div><div className="journal-tile__meta"><span>{entry.index} / {entry.category}</span><span>{entry.date}</span></div><h3>{entry.title}</h3><p>{entry.excerpt}</p></Link>)}</div></div></section>
